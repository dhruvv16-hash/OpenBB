...
def test_chartlibrary_pattern_similarity_fetcher(monkeypatch) -> None:
    ...
    async def mock_aextract_data(query, credentials, **kwargs) -> list[dict]:
...
def test_chartlibrary_pattern_similarity_normalizes_response_fields() -> None:
    ...
    # Test rank is 0 is not overridden by index
    results_rank_zero = ChartLibraryPatternSimilarityFetcher.transform_data(
        query,
        [
            {
                "ticker": "msft",
    ...
    assert len(results_rank_zero) == 1
    assert results_rank_zero[0].rank == 0
